...
export function Sidebar({ open = false, onClose }: SidebarProps) {
  ...
  // Desktop-only collapse to an icon rail — persisted so it survives
  // navigation/reloads. Mobile always uses the full drawer (ignored there).
  const [collapsed, setCollapsed] = useState(false);
  useEffect(() => {
    if (window.localStorage.getItem("nexoomni:sidebar-collapsed") === "1") setCollapsed(true);
  }, []);
  const toggleCollapsed = () => {
    setCollapsed((prev) => {
      const next = !prev;
      window.localStorage.setItem("nexoomni:sidebar-collapsed", next ? "1" : "0");
      return next;
    });
  };
        ...
        <nav className="flex-1 overflow-y-auto px-3 py-4">
              ...
              return (
                <li key={item.href}>
                  ...
                  >
                    ...
                    {collapsed && (showUnreadDot || showNotificationBadge || showCallTaskBadge) ? (
                    ...
                    {showNotificationBadge && (
                      <span
                        ...
                        className={cn("flex h-5 min-w-5 items-center justify-center rounded-full bg-primary px-1 text-[10px] font-semibold text-primary-foreground", collapsed && "lg:hidden")}
                      >
                    ...
                    {showCallTaskBadge && (
                      <span
                        ...
                        className={cn("relative flex h-5 min-w-5 items-center justify-center rounded-full bg-red-500 px-1 text-[10px] font-semibold text-white", collapsed && "lg:hidden")}
                      >
          ...
          {isPlatformOperator ? (
            <>
              ...
              <Link
                href="/platform"
                title={collapsed ? t('platformAdministration') : undefined}
                className={cn(
              ...
              >
                ...
                <span className={cn("flex-1", collapsed && "lg:hidden")}>{t('platformAdministration')}</span>
                <Shield className={cn("h-3.5 w-3.5", collapsed && "lg:hidden")} />
              </Link>
            </>
          ) : null}
