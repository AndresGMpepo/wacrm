...
import { CheckCircle2, Copy, Loader2, Pause, Play, Save, Share2, Trash2 } from 'lucide-react'
...
export function MetaMessagingConfig() {
  ...
  const manage = async (connector: Connector, action: 'pause' | 'resume' | 'delete') => {
    ...
    try {
      const response = await fetch(`/api/omnichannel/meta/connectors/${connector.id}`, action === 'delete' ? { method: 'DELETE' } : { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action }) })
      const data = await response.json().catch(() => ({}))
  ...
  return <Card>
    <CardHeader><CardTitle className="flex items-center gap-2"><Share2 className="size-5" />Facebook e Instagram</CardTitle><CardDescription>Conecta Messenger e Instagram Direct por empresa. Las credenciales se cifran en el servidor y nunca se muestran nuevamente.</CardDescription></CardHeader>
