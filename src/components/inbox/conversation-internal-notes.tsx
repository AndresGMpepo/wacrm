...
import { ChevronDown, ChevronUp, Loader2, LockKeyhole, Send, StickyNote } from 'lucide-react'
import { toast } from 'sonner'
import { Button } from '@/components/ui/button'
import { Textarea } from '@/components/ui/textarea'
import { createClient } from '@/lib/supabase/client'

type Note = { id: string; author_user_id: string; author_name: string; body: string; created_at: string }
...
export function ConversationInternalNotes({ conversationId }: { conversationId: string }) {
  ...
  return <section className="border-t border-border bg-card px-4 py-3">
    ...
    {expanded ? <div className="mt-3 space-y-2"><div className="max-h-36 space-y-2 overflow-y-auto">{loading ? <p className="text-xs text-muted-foreground">Cargando notas…</p> : notes.length ? notes.map((note) => <article key={note.id} className="rounded-md border border-amber-500/20 bg-amber-500/5 p-2 text-xs"><p className="font-medium text-foreground">{note.author_name}<span className="ml-1 font-normal text-muted-foreground">· {new Date(note.created_at).toLocaleString()}</span></p><p className="mt-1 whitespace-pre-wrap text-muted-foreground">{note.body}</p></article>) : <p className="text-xs text-muted-foreground">Aún no hay indicaciones internas.</p>}</div><div className="flex items-end gap-2"><Textarea value={body} onChange={(event) => setBody(event.target.value)} placeholder="Escribe una indicación para el equipo…" rows={2} maxLength={2000} disabled={saving} /><Button size="icon" onClick={() => void addNote()} disabled={!body.trim() || saving} title="Guardar nota interna">{saving ? <Loader2 className="size-4 animate-spin" /> : <Send className="size-4" />}</Button></div></div> : null}
  </section>
}
