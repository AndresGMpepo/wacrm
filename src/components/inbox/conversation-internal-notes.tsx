...
export function ConversationInternalNotes({ conversationId, compact = false, openSignal }: { conversationId: string; compact?: boolean; openSignal?: number }) {
  ...
  const markRead = useCallback(async () => {
    try {
      const response = await fetch(`/api/conversations/${conversationId}/internal-notes`, { method: 'PATCH' })
      if (response.ok) window.dispatchEvent(new Event(`internal-note-read:${conversationId}`))
    } catch {
      // The visible notes remain usable even if marking them read is delayed.
    }
  }, [conversationId])
  ...
  return <section className={compact ? 'rounded-lg border border-border bg-muted/20 p-3' : 'border-t border-border bg-card px-4 py-3'}>
    <div className="flex items-center justify-between gap-3">
      ...
      <Button size="icon" variant="ghost" onClick={toggleExpanded} aria-label={expanded ? 'Ocultar notas internas' : 'Mostrar notas internas'}>{expanded ? <ChevronUp className="size-4" /> : <ChevronDown className="size-4" />}</Button>
    </div>
