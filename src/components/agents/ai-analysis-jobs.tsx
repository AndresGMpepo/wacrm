...
const label: Record<string, string> = { customer_message: 'Mensaje del cliente', transfer: 'Transferencia', close: 'Cierre', manual: 'Manual', queued: 'Pendiente', processing: 'Procesando', completed: 'Completado', skipped_limit: 'Límite alcanzado', failed: 'Fallido' }
...
export function AiAnalysisJobsCard() {
  ...
  const retry = async (id: string) => {
    setRetrying(id)
    try { const response = await fetch('/api/ai/analysis-jobs', { method: 'PATCH', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ job_id: id }) }); const json = await response.json(); if (!response.ok) throw new Error(json.error); toast.success('Trabajo enviado nuevamente a la cola.'); await load() } catch (error) { toast.error(error instanceof Error ? error.message : 'No se pudo reintentar.') } finally { setRetrying(null) }
  }
  return <section className="rounded-xl border bg-card p-5">
    ...
    {data ? <><div className="mt-4 grid gap-3 sm:grid-cols-4"><Metric label="Pendientes" value={data.counts.queued ?? 0} icon={Clock3} /><Metric label="Completados" value={data.counts.completed ?? 0} icon={CheckCircle2} /><Metric label="Fallidos" value={data.counts.failed ?? 0} icon={XCircle} /><Metric label="Omitidos" value={data.counts.skipped_limit ?? 0} icon={AlertTriangle} /></div>
      ...
      <div className="mt-4 divide-y rounded-md border">{data.jobs.length ? data.jobs.map((job) => <div key={job.id} className="flex items-center gap-3 p-3 text-sm"><span className="min-w-0 flex-1"><b>{label[job.status] ?? job.status}</b><span className="block text-xs text-muted-foreground">{label[job.trigger] ?? job.trigger} · {new Date(job.updated_at).toLocaleString()} {job.error_message ? `· ${job.error_message}` : ''}</span></span>{['failed', 'skipped_limit'].includes(job.status) ? <Button size="sm" variant="outline" disabled={retrying === job.id} onClick={() => void retry(job.id)}><RotateCcw className="size-3.5" />Reintentar</Button> : null}</div>) : <p className="p-4 text-sm text-muted-foreground">Aún no hay trabajos en la cola.</p>}</div>
