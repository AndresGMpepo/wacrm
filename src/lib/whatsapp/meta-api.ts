...
async function throwMetaError(response: Response, fallback: string): Promise<never> {
  ...
  try {
    ...
    if (data.error?.message) {
      const detail = [
        typeof data.error.code === 'number' ? `código ${data.error.code}` : null,
        typeof data.error.error_subcode === 'number'
          ? `subcódigo ${data.error.error_subcode}`
          : null,
      ].filter(Boolean).join(', ')
      message = detail ? `${data.error.message} (${detail})` : data.error.message
    }
  } catch {
