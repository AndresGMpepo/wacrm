...
export async function GET() {
  try {
    ...
    const { data, error } = await admin().from('omnichannel_connectors')
      .select('id, display_name, external_channel_id, status, webhook_secret, last_event_at, last_error, created_at, updated_at')
...
export async function POST(request: Request) {
  try {
    ...
    const { data, error } = await query.select('id, display_name, external_channel_id, status, webhook_secret, last_event_at, last_error, created_at, updated_at').single()
    if (error) throw error
