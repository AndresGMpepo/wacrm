...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('google_calendar_connections').select('assigned_agent_id, calendar_id, connected_at').eq('account_id', accountId).order('connected_at', { ascending: false })
