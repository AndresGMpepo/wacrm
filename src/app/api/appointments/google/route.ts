...
export async function GET(request: Request) {
  try {
    ...
    const { data, error } = await supabase.from('google_calendar_connections')
      .select('id, assigned_agent_id, calendar_id, display_name, is_default, connected_at, last_synced_at, last_error')
      .eq('account_id', accountId).order('connected_at', { ascending: false })
    if (error) throw error
    const agentIds = [...new Set((data ?? []).map((row) => row.assigned_agent_id).filter((id): id is string => Boolean(id)))]
    const { data: agents, error: agentsError } = agentIds.length
      ? await supabase.from('profiles').select('user_id, full_name').eq('account_id', accountId).in('user_id', agentIds)
      : { data: [] as { user_id: string; full_name: string }[], error: null }
    if (agentsError) throw agentsError
    const agentNames = new Map((agents ?? []).map((agent) => [agent.user_id, agent.full_name]))
    const connections = (data ?? []).map((row) => ({ ...row, assigned_agent_name: row.assigned_agent_id ? agentNames.get(row.assigned_agent_id) ?? null : null }))
...
export async function POST(request: Request) {
  try {
    ...
    const body = await request.json().catch(() => null) as { assigned_agent_id?: unknown } | null
